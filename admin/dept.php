...
$user_id = $_SESSION['id'];
           ...
           //add teacher modal
            ...
            if(isset($_POST['submittt'])){
                ...
                $select = mysqli_query($conn, "SELECT * FROM `dept` WHERE name = '$name'") or die('query failed');

                if(mysqli_num_rows($select) > 0){
                    $message[] = 'dept already exist'; 
                }else{
                        $insert = mysqli_query($conn, "INSERT INTO `dept`(name) VALUES('$name')") or die('query failed');
                    }
                }
                // PROFILE MODAL
        ?>
                        ...
                        <table class="table bg-white rounded shadow-sm  table-hover">
                         <thead>
                                ...
                                <tbody>
                                    <?php
                                        $i = 1;
                                        $qry = $conn->query("SELECT * FROM dept");
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="updept.php?id=<?php echo $row['id'] ?>" class="btn btn-primary btn-flat ">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a name="delete" href="dept.php?id=<?php echo $row['id'] ?>" class="btn btn-danger btn-flat" onclick="return confirm('Are you sure you want to delete this record?')">
                                    ...
                                    <?php endwhile;
                                        ...
                                        if(isset($_GET['id'])){
                                            $id = $_GET['id'];
                                            $conn->query("DELETE FROM dept WHERE id='".$id."'");
                                            echo '<div class="alert alert-success">dept deleted successfully</div>';
                                        }
                                    ?>
                                </tbody>
