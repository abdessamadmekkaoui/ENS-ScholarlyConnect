...
if(isset($_POST['update_profile'])){
    ...
    $update_loca = mysqli_real_escape_string($conn, $_POST['location']);
    $new_email = mysqli_real_escape_string($conn, $_POST['email']);
    $conf_email = mysqli_real_escape_string($conn, $_POST['cemail']);
    if(!empty($new_email)&&!empty($conf_email)&&!empty($name)){
       if ($new_email!=$conf_email) {
          $message[] = 'conferm email not correct!';
       }else{
    mysqli_query($conn, "UPDATE `teacher` SET name = '$name', location = '$update_loca', email = '$new_email' WHERE id = '$id'") or die('query failed');
    $message[] = 'updated successfully!';
 }
    }elseif(empty($name)){
        $message[] = 'name not inserted!';
     }elseif(empty($new_email)){
       $message[] = 'new email not inserted!';
    }elseif(empty($conf_email)){
       $message[] = 'conferm email not inserted!';
    }
    ...
    if(!empty($new_pass) || !empty($confirm_pass)){
      if($new_pass != $confirm_pass){
          ...
          mysqli_query($conn, "UPDATE `teacher` SET password = '$confirm_pass' WHERE id = '$id'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 2000000){
         ...
         $image_update_query = mysqli_query($conn, "UPDATE `teacher` SET image = '$update_image' WHERE id = '$id'") or die('query failed');
...
<div class="d-flex" id="wrapper">
    ...
    <div class="update-profile">
        ...
        <?php
            $select = mysqli_query($conn, "SELECT * FROM `teacher` WHERE id = '$id'") or die('query failed');
            if(mysqli_num_rows($select) > 0){
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="flex">
                <div class="inputBox">
                    <span>teacher name :</span>
                        <input type="text" name="name" placeholder="enter new name" value="<?php echo $fetch['name']; ?>" class="box">
                    <span>teacher email :</span>
                        <input type="text" name="email" placeholder="enter new eamil" class="box">
                    <span>conferm email :</span>
                        <input type="text" name="cemail" placeholder="conferm new eamil" class="box">
                </div>
                <div class="inputBox">
                    <span>location :</span>
                        <input type="text" name="location" placeholder="enter new location" value="<?php echo $fetch['location']; ?>" class="box">
                    <span>new password :</span>
                        <input type="password" name="new_pass" placeholder="enter new password" class="box">
                    <span>confirm password :</span>
                        <input type="password" name="confirm_pass" placeholder="confirm new password" class="box">
            ...
            <a href="teachers.php" class="delete-btn">go back</a>
        </form>
    </div>
</div>
